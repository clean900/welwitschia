...
<template>
    ...
    <AppLayout>
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-extrabold text-white">Compras</h1>
            <Link href="/app/compras/criar" class="px-4 py-2 rounded-lg bg-gradient-to-r from-emerald-500 to-emerald-600 text-white font-semibold hover:opacity-90">Nova ordem de compra</Link>
        </div>
        <div class="bg-[#121829] border border-white/5 rounded-2xl overflow-hidden">
            <table class="w-full text-sm">
                <thead class="text-slate-500 text-xs uppercase border-b border-white/5">
                    <tr><th class="text-left px-5 py-3">Número</th><th class="text-left px-5 py-3">Fornecedor</th><th class="text-right px-5 py-3">Total</th><th class="text-center px-5 py-3">Estado</th><th class="px-5 py-3"></th></tr>
                </thead>
                <tbody>
                    <tr v-for="o in orders.data" :key="o.id" class="border-b border-white/5 hover:bg-white/5">
                        ...
                        <td class="px-5 py-3 text-slate-200">{{ o.supplier || '—' }}</td>
                        ...
                        <td class="px-5 py-3 text-right"><Link :href="`/app/compras/${o.id}`" class="text-emerald-400 hover:underline">Abrir</Link></td>
                    </tr>
                    <tr v-if="orders.data.length === 0"><td colspan="5" class="px-5 py-12 text-center text-slate-500">Sem ordens de compra.</td></tr>
                </tbody>
            </table>
        </div>
    </AppLayout>
