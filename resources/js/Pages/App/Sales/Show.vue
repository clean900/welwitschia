...
const badge = { draft: 'bg-slate-600/30 text-slate-300', confirmed: 'bg-amber-500/20 text-amber-300', invoiced: 'bg-emerald-500/20 text-emerald-300', cancelled: 'bg-pink-500/20 text-pink-300' };
const label = { draft: 'Rascunho', confirmed: 'Confirmada', invoiced: 'Faturada', cancelled: 'Cancelada' };

function confirmOrder() { router.post(`/app/vendas/${props.order.id}/confirmar`); }
function invoice() { router.post(`/app/vendas/${props.order.id}/facturar`); }
</script>
...
<template>
    ...
    <AppLayout>
        <div class="flex items-center justify-between mb-6">
            <div>
                <Link href="/app/vendas" class="text-sm text-slate-500 hover:text-slate-300">← Vendas</Link>
        ...
        <div class="grid gap-6 md:grid-cols-3">
            <div class="md:col-span-2 bg-[#121829] border border-white/5 rounded-2xl p-6">
                <div class="text-sm text-slate-400 mb-4">Cliente: <span class="text-slate-100 font-medium">{{ order.customer_name || '—' }}</span></div>
                <table class="w-full text-sm mb-4">
                    <thead class="text-slate-500 text-xs uppercase border-b border-white/5">
                        <tr><th class="text-left py-2">Descrição</th><th class="text-right py-2">Qtd</th><th class="text-right py-2">Preço</th><th class="text-right py-2">IVA</th><th class="text-right py-2">Total</th></tr>
                    </thead>
                    <tbody>
            ...
            <div class="bg-[#121829] border border-white/5 rounded-2xl p-6 space-y-4">
                ...
                <template v-if="order.status === 'draft'">
                    <button @click="confirmOrder" class="w-full py-2 rounded-lg bg-gradient-to-r from-emerald-500 to-emerald-600 text-white font-semibold hover:opacity-90">Confirmar encomenda</button>
                </template>
                <template v-else-if="order.status === 'confirmed'">
                    <button @click="invoice" class="w-full py-2 rounded-lg bg-gradient-to-r from-emerald-500 to-emerald-600 text-white font-semibold hover:opacity-90">Faturar (gera factura + saída de stock)</button>
                </template>
                <template v-else-if="order.status === 'invoiced'">
                    <div class="text-sm bg-emerald-500/10 border border-emerald-500/30 rounded-lg p-3 text-emerald-300">Encomenda faturada ✓</div>
                    <Link v-if="order.invoice_id" :href="`/app/invoices/${order.invoice_id}`" class="block text-center py-2 rounded-lg border border-white/10 text-slate-300 hover:bg-white/5">Ver factura</Link>
                </template>
            </div>
        </div>
    </AppLayout>
