...
<template>
    <Head title="Entrar — Welwitschia ERP" />

    <div class="min-h-screen bg-stone-100 flex items-center justify-center px-6">
        <div class="w-full max-w-sm">
            <Link href="/" class="block text-center mb-8">
                <span class="text-xl font-extrabold">Welwitschia <span class="text-orange-700">ERP</span></span>
            </Link>

            <form @submit.prevent="submit" class="bg-white border border-stone-200 rounded-xl p-8 space-y-4">
                <h1 class="text-lg font-bold mb-2">Entrar na sua empresa</h1>

                <div>
                    <label class="block text-sm font-medium mb-1">Email</label>
                    <input v-model="form.email" type="email" class="field" autofocus autocomplete="username" />
                    <p v-if="form.errors.email" class="err">{{ form.errors.email }}</p>
                </div>

                <div>
                    <label class="block text-sm font-medium mb-1">Palavra-passe</label>
                    <input v-model="form.password" type="password" class="field" autocomplete="current-password" />
                ...
                <p class="text-center text-sm text-stone-500 pt-2">
                    Ainda não tem conta?
                    <Link href="/registar-empresa" class="text-orange-700 font-medium">Registar empresa</Link>
                </p>
            </form>
        </div>
    </div>
